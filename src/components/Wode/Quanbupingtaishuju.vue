  ...
  methods: {
    async getData() {
      ...
      this.tableData.forEach((ele) => {
        ...
        ele.zongAdd = `${ele.address.province} ${ele.address.city} ${ele.address.area} ${ele.address.address}`;
      });
    ...
    async exportSelectEvent() {
      if (this.titVal != "全部订单") {
        const res = await this.$api.orderOrder_by_time(this.val, 1);
        console.log(res);
    ...
    async seeDingdan(val) {
      ...
      const res = await this.$api.orderOrder_by_time(val);
      console.log(res);
    ...
    onReset() {
      this.$store.commit("quanbupingtaishujuPage", 1);
